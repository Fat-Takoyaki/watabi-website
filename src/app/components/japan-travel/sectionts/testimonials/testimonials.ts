...
interface TestimonialsContent {
  ...
  cta: {
    text: string;
    buttonText: string;
    buttonRoute: string; // es: "/recensioni"
  };
}
...
export class Testimonials implements OnInit {
  content: TestimonialsContent | null = null;
  ...
  private loadContent(): void {
    this.contentService
      .loadContent<TestimonialsContent>('testimonials')
      .subscribe({
        next: (data) => {
  ...
  navigateToReviews(): void {
    const route = this.content?.cta?.buttonRoute || '/recensioni';
  ...
  private getEmergencyContent(): TestimonialsContent {
    return {
      header: {
        title:
          'Cosa dicono i viaggiatori che hanno scelto un <span class="text-red-watabi">viaggio su misura</span>',
        subtitle:
          'Le testimonianze autentiche di chi ha già vissuto la propria esperienza giapponese con Watabi',
      },
      testimonials: [
        {
          name: 'Marco e Giulia',
      ...
      cta: {
        text: 'Vuoi leggere altre esperienze di viaggio?',
        buttonText: 'Leggi tutte le recensioni',
        buttonRoute: '/recensioni',
      },
    };
  }
}
