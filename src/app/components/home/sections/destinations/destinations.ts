...
export class Destinations implements OnInit {
  content: DestinationsContent | null = null;
  ...
  private loadContent(): void {
    this.contentService
      .loadContent<DestinationsContent>('destinations')
      .subscribe({
        next: (data) => {
          if (data) {
            this.content = data;
        ...
        error: (err) => {
          console.error('Errore caricamento Destinations:', err);
          this.content = this.getEmergencyContent();
        },
      });
  }

  navigateToBlog(route: string): void {
    this.router.navigate([route]);
  }

  private getEmergencyContent(): DestinationsContent {
    return {
      header: {
        title:
          'Luoghi <span class="text-red-watabi">indimenticabili</span> da esplorare',
        description:
          'Dal fermento di Tokyo alla spiritualità di Kyoto, dalle isole tropicali di Okinawa alla maestosità del Monte Fuji: il Giappone offre destinazioni per ogni tipo di viaggiatore. Scopri i luoghi che renderanno unico il tuo viaggio.',
      },
      destinations: [
        {
          id: 'tokyo',
      ...
      bottomCta: {
        title: 'Vuoi esplorare altre destinazioni?',
        description:
          'Il Giappone ha molto altro da offrire: dalle Alpi Giapponesi a Hiroshima, da Takayama a Nikko. Scopri tutte le destinazioni nella nostra guida completa.',
        buttonText: 'Guida completa alle destinazioni',
        buttonLink: '/blog/destinazioni-giappone',
      },
    };
  }
}
